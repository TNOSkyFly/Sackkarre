...
<head>
    ...
    <style>
        ...
        /* Status Farben */
        .state-0 { color: #ff4444 !important; } /* AUS - Rot */
        .state-1 { color: #ffcc00 !important; } /* BEREIT - Gelb */
        .state-2 { color: #00ffcc !important; } /* AKTIV - Türkis */
...
<body>
    ...
    <div class="diag-card" style="margin-bottom:15px;"> 
    ...
    <script>
        ...
        setInterval(() => {
            fetch('/status').then(r => r.json()).then(d => {
                ...
                st.innerHTML = states[d.s];
                st.className = "state-" + d.s;
                ...
                const st = document.getElementById('st-txt');
                st.innerHTML = "VERBINDUNG VERLOREN";
                st.style.color = "red";
            });
        }, 200);
...
bool get_gas(int &pwm) {
    ...
    if (minPoti != lastLoggedMin || maxPoti != lastLoggedMax) {
        Serial.printf("Poti: %d | Min: %d | Max: %d\n", rawValue, minPoti, maxPoti);
        lastLoggedMin = minPoti; lastLoggedMax = maxPoti;
    }

    // SICHERHEIT: Kabelbruch-Erkennung (Pull-Down am Pin vorausgesetzt)
